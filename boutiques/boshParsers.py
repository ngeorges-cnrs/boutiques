...
def add_subparser_data(subparsers):
    ...
    parser_data_delete.add_argument(
        "-f", "--file", action="store", help="Filename of record to delete."
    )
    parser_data_delete.add_argument(
        "--no-int",
        "-y",
    ...
    parser_data_inspect.add_argument(
        "-e",
        "--example",
        action="store_true",
    ...
    parser_data_publish.add_argument(
        ...
        "--file",
        action="store",
        help="Filename of record to publish alone as a " "data set.",
    )
    parser_data_publish.add_argument(
        "-i",
        "--individually",
    ...
    parser_data_publish.add_argument(
        "--nexus-project", action="store", help="Nexus project to publish to. "
    )
...
def add_subparser_deprecate(subparsers):
    parser_deprecate = subparsers.add_parser(
        "deprecate",
        description="Deprecates a published descriptor by"
...
def add_subparser_example(subparsers):
    parser_example = subparsers.add_parser(
        "example",
        description="Generates example invocation from a" " valid descriptor",
    )
...
def add_subparser_execute(subparsers):
    ...
    parser_exec_launch.add_argument(
        ...
        "--stream",
        action="store_true",
        help="Streams stdout and stderr in real time " "during execution.",
    )
    parser_exec_launch.add_argument(
        "--imagepath",
        action="store",
    ...
    parser_exec_launch.add_argument(
        ...
        action="store_true",
        help="Skips execution data collection and saving" "to cache.",
    )
    parser_exec_launch.add_argument(
        "--provenance",
        action="store",
...
def add_subparser_export(subparsers):
    parser_export = subparsers.add_parser(
        "export",
        description="Export Boutiques descriptor to" " other formats.",
    )
    ...
    parser_export.add_argument(
        "output",
        help="Output file where to write the" " converted descriptor.",
    )
...
def add_subparser_import(subparsers):
    parser_import = subparsers.add_parser(
        "import",
        description="Imports old descriptor or BIDS app or"
    ...
    parser_import.add_argument(
        "-o",
        "--output-invocation",
        help="Where to write " "the invocation if any.",
    )
    parser_import.add_argument(
        "-i", "--input-invocation", help="Input invocation " " for CWL if any."
    )
...
def add_subparser_invocation(subparsers):
    parser_invocation = subparsers.add_parser(
        "invocation",
        description="Creates invocation schema and"
...
def add_subparser_pprint(subparsers):
    parser_pprint = subparsers.add_parser(
        "pprint",
        description="Boutiques pretty-print for" "generating help text",
    )
    parser_pprint.set_defaults(function="pprint")
    parser_pprint.add_argument(
        "descriptor", action="store", help="The Boutiques descriptor."
    )
    parser_pprint.add_argument(
        "--sandbox",
        action="store_true",
...
def add_subparser_publish(subparsers):
    ...
    parser_publish.add_argument(
        ...
        action="store",
        help="local path of the " " Boutiques descriptor to publish.",
    )
    parser_publish.add_argument(
        "--sandbox",
        action="store_true",
...
def add_subparser_pull(subparsers):
    parser_pull = subparsers.add_parser(
        "pull",
        description="Ensures that Zenodo descriptors are"
...
def add_subparser_search(subparsers):
    parser_search = subparsers.add_parser(
        "search",
        description="Search Zenodo for Boutiques"
...
def add_subparser_validate(subparsers):
    ...
    parser_validate.add_argument(
        ...
        "-f",
        action="store_true",
        help="If descriptor is valid, rewrite it with sorted" " keys.",
    )
    parser_validate.add_argument(
        "--sandbox",
        action="store_true",
